...
export const setActiveCategoryAtom = atom(null, (_, set, categoryId: string) => {
  set(settingsStateAtom, prev => ({
    ...prev,
...
export const setActiveFieldAtom = atom(null, (_, set, fieldKey: string | null) => {
  set(settingsStateAtom, prev => ({
    ...prev,
...
export const updatePreviewValueAtom = atom(null, (_, set, { key, value }: { key: string, value: unknown }) => {
  set(settingsStateAtom, prev => ({
    ...prev,
    previewValues: {
      ...prev.previewValues,
...
export const resetToDefaultsAtom = atom(null, (_, set) => {
  ...
  const { SETTINGS_CATEGORIES } = require('@features/settings/config/settingsDefinitions');
...
export const saveSettingsAtom = atom(null, async (get, set) => {
  ...
  try {
    ...
    const { saveConfigToFile } = await import('@features/settings/utils/configSaver');
