...
export function ModalManager({
  ...
  const handleDataExport = useCallback(
    async (options: ExportOptions) => {
      try {
        const filename = generateDefaultFilename(options.format);
        await exportToFile(displayData, options.format, filename, options);
    ...
    <Box flexDirection="column" width="100%">
      ...
      {exportDialog.isVisible && (
        <ExportDialog
          ...
          onExport={exportHandlers.handleSchemaExport}
          onClose={exportHandlers.closeSchemaExportDialog}
          height={terminalSize.height}
          width={terminalSize.width}
        />
      )}
      ...
      {dataExportDialog.isVisible && (
        <ExportDialog
          ...
          onClose={() => setDataExportDialog({ isVisible: false })}
          height={terminalSize.height}
          width={terminalSize.width}
        />
      )}
