...
export function DebugLogViewer({ height, width, onExit }: DebugLogViewerProps) {
  ...
  const [selectedLevel, setSelectedLevel] = useState<DebugLogLevel | null>(null);
  ...
  // 表示するログのスライス
  const visibleLogs = filteredLogs.slice(scrollOffset, scrollOffset + visibleLines);
  ...
  useInput((input, key) => {
    ...
    if (key?.upArrow || input === 'k') {
      ...
      const newIndex = Math.min(filteredLogs.length - 1, selectedIndex + visibleLines);
      setSelectedIndex(newIndex);
      setScrollOffset(Math.min(maxScroll, newIndex - Math.floor(visibleLines / 2)));
    } else if (input === 'c') {
      // フィルタをクリア
    ...
    <Box flexDirection="column" width={width} height={height}>
      ...
      <Box width={width} borderStyle="single" borderColor="blue">
        <Text bold color="blue">
          DEBUG LOG VIEWER ({filteredLogs.length} entries) - q:quit, c:clear filter, C:clear all, r:refresh
      ...
      <Box width={width}>
        <Text>
          Total: {stats.total} | 
          ERROR: <Text color="red">{stats.byLevel.error}</Text> | 
          WARN: <Text color="yellow">{stats.byLevel.warn}</Text> | 
          INFO: <Text color="blue">{stats.byLevel.info}</Text> | 
          DEBUG: <Text color="gray">{stats.byLevel.debug}</Text>
      ...
      <Box width={width} borderStyle="single" borderColor="blue">
        <Text>
          ...
          j/k: navigate, PageUp/PageDown: page scroll, gg: go to top, G: go to bottom, r: go to latest
        </Text>
      </Box>
    </Box>
  );
